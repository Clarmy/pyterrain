"""CLI entrypoints for pyterrain helper commands."""
...
SKILL_NAME = "pyterrain-python-assistant"
CODEX_MARKER_START = "<!-- pyterrain-codex-skill:start -->"
CODEX_MARKER_END = "<!-- pyterrain-codex-skill:end -->"
...
def get_codex_skill_source_dir() -> Path:
    ...
    bundled_skill_dir = _project_root() / "pyterrain" / "_bundled_skills" / "platforms" / "codex" / SKILL_NAME
...
def get_cursor_skill_entry_dir() -> Path:
    ...
    bundled_skill_dir = _project_root() / "pyterrain" / "_bundled_skills" / "platforms" / "cursor" / SKILL_NAME
    if (bundled_skill_dir / "SKILL.md").exists():
        return bundled_skill_dir
...
def get_claudecode_skill_entry_dir() -> Path:
    """Return the bundled Claude Code skill entry directory."""
    bundled_skill_dir = _project_root() / "pyterrain" / "_bundled_skills" / "platforms" / "claudecode" / SKILL_NAME
    if (bundled_skill_dir / "SKILL.md").exists():
        return bundled_skill_dir
...
def get_shared_skill_support_dir() -> Path:
    ...
    bundled_skill_dir = _project_root() / "pyterrain" / "_bundled_skills" / "shared" / SKILL_NAME
...
def install_claudecode_skill(workspace: Path, force: bool = False, scope: str = "local") -> list[Path]:
    entry_dir = get_claudecode_skill_entry_dir()
    shared_dir = get_shared_skill_support_dir()
    install_root = _resolve_install_root(workspace, scope)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="pyterrain")
    subparsers = parser.add_subparsers(dest="command")

    install_parser = subparsers.add_parser(
        "install-skill",
        help="Install pyterrain AI guidance into the current project for a supported assistant.",
    )
    ...
    install_parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite files created by a previous pyterrain skill installation.",
    )
...
def main(argv=None) -> int:
    ...
    if args.command == "install-skill":
        ...
        lines = [f"Installed pyterrain {args.assistant} guidance in {args.mode} mode under {target_root}:"]
